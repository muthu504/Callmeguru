 ...
 * @author shiva
 */
@WebServlet(name = "StoreProvider", urlPatterns = {"/StoreProvider"})
public class StoreProvider extends HttpServlet {
 ...
 getServletContext().getRequestDispatcher("/Header.html").include(request, response);
            ...
            String ss = request.getParameter("OpService");
            String sad = request.getParameter("txtAddrs");
            String query = "insert into providers(pid,username,password,email,phone,typeofservice,address) values(?,?,?,?,?,?,?)";
            PreparedStatement p1 = con.prepareStatement(query);
            p1.setInt(1, DataBase.getNextId("providers"));
            p1.setString(2, su);
            p1.setString(3, sp);
            p1.setString(4, se);
            p1.setString(5, sph);
            ...
            con.close();
            out.print("<h1 align=center><i>Successfully Registered.Please <a href=/CallMeGuru/ProviderLogin.html>Login</a></i></h1>");
